...
echo "get origin md5sum success"
# 2. check md5sum and make child process crash
rm -f $bootstrap_path
rm -f $datablob_path
sleep 2
(./test-private-01-daemon $fscachedir > daemon.log &)
sleep 2

mount -t erofs none -o fsid=test.img -o device=blob1.img /mnt/ > /dev/null 2>&1
if [ $? -ne 0 ]; then
	echo "test-private-01-daemon mount failed"
	pkill test-private-01-daemon
	exit 1
fi
...
#cleanup
killall test-private-01-daemon
pkill cache-child
sleep 1
rm -f *.img
rm -f md5.log
